...
export default function AdminGrossePrise() {
  ...
  const handleSaveEntry = async (saveAndNext = false) => {
    ...
    const entryData = {
      ...
      grossePrise: parseInt(biggestCatch),
    ...
    try {
      ...
      console.log('Big catch data sent to Firebase:', {
        id: entryData.id,
        sector: entryData.sector,
        competitorId: entryData.competitorId,
        biggestCatch: entryData.biggestCatch,
        status: entryData.status
      });
    } catch (error) {
      console.error('Error saving big catch to Firebase:', error instanceof Error ? error.message : error);
      alert('Erreur lors de la sauvegarde dans Firebase: ' + (error instanceof Error ? error.message : error));
      setIsSaving(false);
      return;
    }
