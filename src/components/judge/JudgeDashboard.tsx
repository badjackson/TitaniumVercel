...
export default function JudgeDashboard() {
  ...
  const competitionStarted = useMemo(() => {
    return currentTime >= competitionStartTime;
  }, [currentTime, competitionStartTime]);

  const timeSinceStart = useMemo(() => {
    return currentTime.getTime() - competitionStartTime.getTime();
  }, [currentTime, competitionStartTime]);

  const currentHour = useMemo(() => {
    return competitionStarted ? Math.max(1, Math.floor(timeSinceStart / (1000 * 60 * 60)) + 1) : 1;
  }, [competitionStarted, timeSinceStart]);

  const timeUntilStart = useMemo(() => {
    return competitionStartTime.getTime() - currentTime.getTime();
  }, [competitionStartTime, currentTime]);

  const judgeSector = useMemo(() => {
    return currentUser?.sector || 'A';
  }, [currentUser?.sector]);
  ...
  // Timer effect - MUST be after all other hooks
  useEffect(() => {
    let timerMounted = true;
    ...
    return (
      <div className="h-screen flex items-center justify-center">
        <div className="text-center">
          <Loader2 className="w-8 h-8 mx-auto mb-4 text-ocean-600 animate-spin" />
          <p className="text-gray-600 dark:text-gray-300">
            Chargement des données du juge...
          </p>
        </div>
      </div>
    );
