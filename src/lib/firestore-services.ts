...
export class CompetitorService {
  ...
  static subscribeToCompetitors(callback: (competitors: CompetitorDoc[]) => void) {
    try {
      return onSnapshot(
        query(collection(db, COLLECTIONS.COMPETITORS), orderBy('boxNumber', 'asc')),
        (snapshot) => {
...
export class HourlyEntryService {
  ...
  static subscribeToHourlyEntries(callback: (entries: HourlyEntryDoc[]) => void) {
    try {
      return onSnapshot(
        query(collection(db, COLLECTIONS.HOURLY_ENTRIES), orderBy('hour', 'asc')),
        (snapshot) => {
      ...
      console.error('Error setting up hourly entries subscription:', error);
      callback([]);
      return () => {};
    }
  }
...
export class BigCatchService {
  ...
  static subscribeToBigCatches(callback: (entries: BigCatchDoc[]) => void) {
    try {
      return onSnapshot(
        query(collection(db, COLLECTIONS.BIG_CATCHES), orderBy('grossePrise', 'desc')),
        (snapshot) => {
          const entries = snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data()
      ...
      console.error('Error setting up big catches subscription:', error);
      callback([]);
      return () => {};
    }
  }
...
export class JudgeService {
  ...
  static subscribeToJudges(callback: (judges: JudgeDoc[]) => void) {
    try {
      return onSnapshot(
        query(collection(db, COLLECTIONS.JUDGES), orderBy('name', 'asc')),
        (snapshot) => {
      ...
      console.error('Error setting up judges subscription:', error);
      callback([]);
      return () => {};
    }
  }
...
export class CompetitionSettingsService {
  ...
  static subscribeToSettings(callback: (settings: CompetitionSettingsDoc | null) => void) {
    try {
      ...
      console.error('Error setting up competition settings subscription:', error);
      callback(null);
      return () => {};
    }
  }
...
export class PublicAppearanceService {
  ...
  static subscribeToSettings(callback: (settings: PublicAppearanceSettingsDoc | null) => void) {
    try {
      ...
      console.error('Error setting up public appearance settings subscription:', error);
      callback(null);
      return () => {};
    }
  }
