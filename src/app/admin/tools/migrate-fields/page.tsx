...
export default function MigrateFieldsPage() {
  ...
  const handleMigrateFields = async () => {
    ...
    try {
      ...
      const docs = competitorsSnapshot.docs;
      ...
      for (const docSnapshot of docs) {
        ...
        try {
          ...
          if (data.hasOwnProperty('poidsTotal')) {
            const poidsTotalValue = data.poidsTotal || 0;
            
            // Update document: add new field and remove old field
            await updateDoc(doc(db, 'competitors', docId), {
              poidsTotalGlobal: poidsTotalValue,
              poidsTotal: deleteField()
            });
            
            migrated++;
            details.push(`✅ Migré: ${data.fullName || docId} (${data.poidsTotal || 0}g → poidsTotalGlobal)`);
          } else if (data.hasOwnProperty('poidsTotalGlobal')) {
            skipped++;
            details.push(`⏭️ Déjà migré: ${data.fullName || docId}`);
          } else {
            // Document doesn't have either field, add poidsTotalGlobal with 0
            await updateDoc(doc(db, 'competitors', docId), {
              poidsTotalGlobal: 0
            });
            
            migrated++;
            details.push(`✅ Ajouté poidsTotalGlobal: ${data.fullName || docId} (0g)`);
          }
        } catch (error: any) {
          errors++;
          details.push(`❌ Erreur ${data.fullName || docId}: ${error.message}`);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">
          Migration des Champs Firebase
        </h1>
        <p className="text-gray-600 dark:text-gray-300">
          Outil pour migrer le champ `poidsTotal` vers `poidsTotalGlobal`
        </p>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700 dark:text-blue-300">
                Cet outil va automatiquement renommer tous les champs `poidsTotal` en `poidsTotalGlobal` dans Firebase.
              </p>
            </div>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              ...
              <span>Migration: poidsTotal → poidsTotalGlobal</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              ...
              <div className="space-y-2 text-sm text-blue-700 dark:text-blue-300">
                <div className="flex items-center space-x-2">
                  ...
                  <span>Renomme le champ `poidsTotal` en `poidsTotalGlobal`</span>
                </div>
                <div className="flex items-center space-x-2">
                  <ArrowRight className="w-4 h-4" />
                ...
                <div className="flex items-center space-x-2">
                  <ArrowRight className="w-4 h-4" />
                  <span>Ajoute `poidsTotalGlobal: 0` si le champ n'existe pas</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
              ...
              <div className="space-y-1 text-sm text-yellow-700 dark:text-yellow-300">
                ...
                <div>• Cette opération modifie tous les documents competitors</div>
                ...
                {migrationResult.success && (
                  <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    ...
                    <p className="text-sm text-green-700 dark:text-green-300">
                      Tous les documents utilisent maintenant le champ `poidsTotalGlobal`. 
            ...
            <div className="space-y-3 text-sm">
              <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                ...
                <p className="text-blue-700 dark:text-blue-300">
                  Assurez-vous que tous les compétiteurs ont des données cohérentes dans Firebase.
                </p>
              </div>
